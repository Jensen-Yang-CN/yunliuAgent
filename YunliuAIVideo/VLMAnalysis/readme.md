...
系统已集成RAG本地知识库功能，可以将视频分析结果与知识库进行比对，只输出知识库中存在的隐患或违章行为。配置步骤如下：
...
2. 确保Milvus向量数据库已启动（默认连接到localhost:19530）

3. 确保已创建知识库集合（默认为enterprise_regulations）

4. 在config.py中配置RAG参数：
```python
class RAGConfig:
    ENABLE_RAG = True  # 启用RAG功能
    MILVUS_HOST = "localhost"  # Milvus服务器地址
    MILVUS_PORT = "19530"  # Milvus服务器端口
    KNOWLEDGE_COLLECTION = "enterprise_regulations"  # 知识库集合名称
    SIMILARITY_THRESHOLD = 0.6  # 相似度阈值
```

5. 运行系统，视频分析结果将与知识库进行比对，只输出匹配的内容

注意：系统不会将视频内容描述存储到向量数据库中，只会保存到本地文件，保证知识库的洁净。视频分析结果仍会保存在本地的video_histroy_info.txt文件中。